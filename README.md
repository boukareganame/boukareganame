<h1 align="center">Hi, I'm Boukaré Ganame 👋</h1>

<p align="center">
  <a href="https://readme-typing-svg.demolab.com">
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&duration=3500&pause=800&color=58A6FF&center=true&vCenter=true&width=600&lines=Développeur+Full-Stack;Ruby+on+Rails+%7C+Django+%7C+Next.js;Récent+diplômé+en+Licence+Informatique;En+route+vers+le+Master+🎓" alt="Typing SVG" />
  </a>
</p>

<p align="center">
  <a href="https://mon-portfolio-sigma-umber.vercel.app">
    <img src="https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfolio" />
  </a>
  <a href="https://www.linkedin.com/in/boukar%C3%A9-ganame-1302b3394">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  <a href="mailto:boukarganame@yahoo.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
  </a>
  <a href="https://github.com/boukareganame">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
  </a>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=boukareganame&label=Profile%20views&color=58A6FF&style=flat-square" alt="Profile views" />
</p>

---

## 🧭 À propos

Développeur Full-Stack basé à **Bobo-Dioulasso, Burkina Faso 🇧🇫**. Je viens de terminer ma **Licence en Informatique** et je m'apprête à entamer un **Master**. Je conçois des applications web complètes — du modèle de données à l'interface — avec une attention particulière pour la qualité du code, l'expérience utilisateur et les architectures modulaires.

J'aime particulièrement les projets qui résolvent un problème concret du contexte local : santé, finance communautaire, éducation, hôtellerie.

---

## 🛠️ Stack technique

### Langages & Frameworks
<p>
  <img src="https://skillicons.dev/icons?i=ruby,rails,python,django,ts,js,nodejs,nextjs,react,tailwind,html,css" alt="Languages & Frameworks" />
</p>

### Bases de données & Outils
<p>
  <img src="https://skillicons.dev/icons?i=postgres,mongodb,redis,docker,git,github,linux,bash,vscode,vercel,figma,postman" alt="Tools" />
</p>

---

## 🚀 Projets en vedette

<table>
  <tr>
    <td width="50%" valign="top">
      <h3>🎓 H-Academy</h3>
      <p>Plateforme e-learning full-stack : <b>API Rails</b> + <b>frontend Next.js</b> dans un monorepo <code>pnpm</code> / <code>Turborepo</code>.</p>
      <p>
        <img src="https://img.shields.io/badge/Rails-CC0000?style=flat-square&logo=rubyonrails&logoColor=white" />
        <img src="https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=next.js&logoColor=white" />
        <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white" />
        <img src="https://img.shields.io/badge/Turborepo-EF4444?style=flat-square&logo=turborepo&logoColor=white" />
      </p>
      <a href="https://github.com/boukareganame/h-academy-frontend">→ Frontend</a> · <a href="https://github.com/boukareganame/h-academy-api">API</a>
    </td>
    <td width="50%" valign="top">
      <h3>🏥 Gestion Hôpital</h3>
      <p>Système de gestion hospitalière développé en <b>stage</b>, Django modulaire : caisse, comptes, laboratoire, patients.</p>
      <p>
        <img src="https://img.shields.io/badge/Django-092E20?style=flat-square&logo=django&logoColor=white" />
        <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" />
        <img src="https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white" />
      </p>
      <a href="https://github.com/boukareganame/gestion_hopital">→ Voir le repo</a>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h3>💰 Tontine</h3>
      <p>Application Rails autour de la <b>tontine</b> — épargne collective traditionnelle ouest-africaine numérisée.</p>
      <p>
        <img src="https://img.shields.io/badge/Rails-CC0000?style=flat-square&logo=rubyonrails&logoColor=white" />
        <img src="https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white" />
      </p>
      <a href="https://github.com/boukareganame/tontine">→ Voir le repo</a>
    </td>
    <td width="50%" valign="top">
      <h3>🎨 Portfolio</h3>
      <p>Mon portfolio personnel construit avec <b>Next.js</b> et déployé sur <b>Vercel</b>.</p>
      <p>
        <img src="https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=next.js&logoColor=white" />
        <img src="https://img.shields.io/badge/Tailwind-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white" />
        <img src="https://img.shields.io/badge/Vercel-000000?style=flat-square&logo=vercel&logoColor=white" />
      </p>
      <a href="https://mon-portfolio-sigma-umber.vercel.app">→ Voir en ligne</a> · <a href="https://github.com/boukareganame/mon-portfolio">Source</a>
    </td>
  </tr>
</table>

---

## 📊 Mon activité GitHub

<p align="center">
  <img height="170" src="https://github-readme-stats.vercel.app/api?username=boukareganame&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" alt="GitHub Stats" />
  <img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=boukareganame&layout=compact&theme=tokyonight&hide_border=true&langs_count=8" alt="Top Languages" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com?user=boukareganame&theme=tokyonight&hide_border=true" alt="GitHub Streak" />
</p>

---

## 🌱 Currently learning

- **Architecture** : design patterns avancés, DDD, hexagonal architecture
- **DevOps** : Docker en production, CI/CD avec GitHub Actions, bases de Kubernetes
- **Cloud** : fondamentaux AWS (EC2, S3, RDS) et alternatives (Vercel, Render, Fly.io)
- **Tests** : RSpec, Pytest, Vitest — discipline du test pour du code maintenable
- **Anglais technique** : pour collaborer plus largement sur l'open-source

---

## 💼 Ouvert à

- 📋 **Stages** et premières opportunités professionnelles
- 🤝 **Collaborations** sur des projets open-source ou à impact local
- 💬 **Échanges techniques** — toujours partant pour apprendre et partager

---

<p align="center">
  <i>« Build it. Ship it. Improve it. »</i><br/>
  <sub>Dernière mise à jour : 2026</sub>
</p>
